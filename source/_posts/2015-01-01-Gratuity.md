...
yes, I managed to make a tip calculator that does not use a keyboard or keypad
at all.
...
will be to compile because Swift and the Watch Extension APIs have changed
...
`UIViewController` transition API. It's a big pain in the ass, but it can really
...
But that is often the trade-off developers need to make. On one hand, you can
...
WatchKit 1.0 was so limiting that I could barely make the app work at all.

### Watch App Version 1.1

In WatchKit 1.0, the only way to make a list scroll with the digital crown was
to have the list be the only list on the screen. This meant I had to make every
step of the app a full-screen view. Also, the List view API did not do cell recycling
as it did on the iPhone. So each list had to be extremely short or else the
screen would take forever to load. Considering the limitations, I think the
results came out well, but I wanted more!
...
way. This was so much better that I was able to remove the tutorial I worked so hard
on in v1.1.
