...
1. Install Git, Homebrew, Python 2
...
1. 50 GB+ to dedicate to storing updates
   - Even the minimal set for 10.4 Tiger is 20 GB
   - Supporting all the way to 10.9 Mavericks takes 200 GB
...
1. Connected to Ethernet or Wi-Fi on the same network as your server Mac
...
So the web server document root and Reposado storage are

`/Volumes/Data/Virtualization/SUS/www`

The Reposado Git clone is

`/Volumes/Data/Virtualization/SUS/reposado`

The Reposado metadata storage is
...
**Give the web server full disk access**
...
`open /Volumes/Data/Virtualization/SUS/reposado/code/preferences.plist` (TextEdit should open)
...
That's it. Now you have to wait for it to download 20 GB or so. You should be able
...
Note that supporting up to 10.10 Yosemite takes about 300 GB of space.
...
web server and the `www` folder are all you need. The Software Update server is
based on a completely static system, which is super nice and reliable.
...
Tailing these logs will show you that the retro Mac is checking all the updates.
...
In the logs on the server Mac, you should see hundreds of file requests to check
